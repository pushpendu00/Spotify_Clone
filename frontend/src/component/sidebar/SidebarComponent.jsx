...
import LibraryArtists from '../shared/LibraryArtists';
import LibraryPlaylist from '../shared/LibraryPlaylist';
import Naviconcomponent from '../shared/naviconcomponent';
...
const SidebarComponent = () => {
    ...
    return (
        <div className='w-full h-full gap-2 flex flex-col'>
            <div className='h-40 flex flex-col justify-center bg-[#262626] p-2 rounded-md'>
                <Naviconcomponent icon='logos:spotify' name="" path="/" />
                <Naviconcomponent icon='ant-design:home-filled' name="Home" path="/" />
                <Naviconcomponent icon='ion:search-outline' name="Search" path="/search" />
            </div>

            {/* Library */}
            {/* max-h-[500px] h-auto min-h-[300px] */}
            <div className={`${loginContext.isAuthenticate?(`${loginContext.masterPlay?('h-[calc(100%-10rem)]'):('h-full')}`):('h-[calc(100%-10rem)]')} library bg-[#262626]  rounded-md flex flex-col bottom-0`}>
                ...
                {loginContext.isAuthenticate === true ?(
                    <div className='w-full library-head 
                        ...
                        <div className='w-full flex items-center justify-around'>
                            <div className='px-2 py-1 bg-[#4d4d4d] rounded-full text-[13px] cursor-pointer'>Playlist</div>
                            <div className='px-2 py-1 bg-[#4d4d4d] rounded-full text-[13px] cursor-pointer'>Artists</div>
                            <div className='px-2 py-1 bg-[#4d4d4d] rounded-full text-[13px] cursor-pointer'>Podcasts</div>
                        </div>
                    </div>
                ):(<></>)}
                ...
                <>
                    <div className={`${loginContext.isAuthenticate?(`${loginContext.masterPlay?('h-[calc(100%-6rem)]'):('h-full')}`):('h-[calc(100%-6rem)]')} overflow-y-scroll scrollbar-thin scrollbar-track-transparent scrollbar-thumb-green-500`}>
